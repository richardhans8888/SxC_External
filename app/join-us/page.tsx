...
export default function JoinUsPage() {
  ...
  const sxcGhostRef = useRef<HTMLSpanElement>(null);

  useEffect(() => {
    const handleScroll = () => {
      const y = window.scrollY;
      if (sxcGhostRef.current) {
        sxcGhostRef.current.style.transform = `translateY(calc(-50% + ${y * 0.15}px))`;
      }
    };
  ...
  return (
    <main className="min-h-screen text-zinc-900 font-sans overflow-x-hidden">

      {/* ══════════════════════════════════════════════════════
          HERO  +  TRANSITION — single continuous dark→light
          ══════════════════════════════════════════════════════ */}
      <div className="relative">
        ...
        <section
          ref={heroRef}
          className="relative h-screen w-full flex flex-col justify-end pb-28 px-6 sm:px-16 overflow-hidden"
        ...
        >
          ...
          <span
            ref={sxcGhostRef}
            className="absolute right-[-2vw] top-1/2 text-[26vw] font-black leading-none select-none pointer-events-none"
          ...
          <div className="relative z-10 max-w-[1400px] mx-auto w-full">
            ...
            <h1 className="text-[14vw] sm:text-[11vw] md:text-[9vw] font-black text-white uppercase leading-[0.88] tracking-tight mb-10">
              Join<br />
            ...
            <div className="flex flex-col sm:flex-row sm:items-end gap-8 sm:gap-20">
              ...
              <div className="flex gap-8 text-white/35 text-xs tracking-widest uppercase font-semibold">
                <a href="#become-member" className="hover:text-blue-400 transition-colors">01 — Membership</a>
                <a href="#volunteer" className="hover:text-blue-400 transition-colors">02 — Volunteer</a>
                <a href="#open-recruitment" className="hover:text-blue-400 transition-colors">03 — Recruit</a>
              </div>
            </div>
          </div>
          ...
          {/* ── GRADIENT BRIDGE: dark → white ───────────────
              This SVG wave + gradient dissolves the hard edge   */}
          <div className="absolute bottom-0 left-0 w-full overflow-hidden leading-[0] z-20" style={{ height: "180px" }}>
            {/* Soft gradient fade */}
        ...
        {/* ── MEMBERSHIP anchored flush below the wave ───── */}
        <section
          id="become-member"
          className="relative w-full min-h-screen flex flex-col lg:flex-row bg-white"
          style={{ marginTop: "-2px" }} // seal any subpixel gap
        >
          {/* Left: text panel */}
          <div className="relative w-full lg:w-[55%] bg-white flex flex-col justify-center px-8 sm:px-16 py-24 lg:py-32">
            {/* Ghost 01 */}
            <span className="absolute top-8 left-4 text-[20vw] lg:text-[13vw] font-black text-zinc-100 leading-none select-none pointer-events-none">
              01
            </span>
      ...
      <section id="volunteer" className="relative w-full min-h-screen bg-zinc-50 flex flex-col lg:flex-row-reverse">
        ...
        {/* Text */}
        <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-16 py-24 lg:py-0">
      ...
      <section id="open-recruitment" className="relative w-full bg-[#0d0d0d] text-white overflow-hidden">
        ...
        <div className="relative z-10 px-8 sm:px-16 pt-24 max-w-[1400px] mx-auto">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-10 mb-16">
            ...
            <p className="text-zinc-400 text-lg max-w-md leading-relaxed lg:text-right">
              Ready to lead? We recruit annually for management roles and on a rolling basis for project positions.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border-t border-white/10">
            {[
              { num:"A", title:"Board of Management", badge:"Annual", color:"blue", desc:"Lead strategic initiatives and manage core divisions. Drive the organisation's direction from the top.", cta:"Learn More" },
              { num:"B", title:"Project Officers", badge:"Rolling", color:"green", desc:"Own specific events and short-term projects end-to-end. Ideal for hands-on executors.", cta:"Apply Now" },
              { num:"C", title:"Division Staff", badge:"Seasonal", color:"blue", desc:"Embed within a division and develop deep domain expertise. Great for specialists.", cta:"Explore Roles" },
            ].map(({ num, title, badge, color, desc, cta }) => (
              <div key={title} className="border-r border-white/10 last:border-r-0 p-10 flex flex-col gap-6 group/card hover:bg-white/[0.06] transition-colors duration-300 cursor-pointer">
                <div className="flex items-start justify-between">
                  <span className="text-5xl font-black text-white/10 leading-none">{num}</span>
                  <span className={`text-xs px-3 py-1 font-bold tracking-widest uppercase ${color === "green" ? "bg-green-500/10 text-green-400" : "bg-blue-500/10 text-blue-400"}`}>
                    {badge}
                  </span>
                </div>
                <div>
